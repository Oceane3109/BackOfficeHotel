...
public class ReservationController {
    ...
    public ModelView list(@RequestParam("date") String dateValue, @RequestParam("waitTime") String waitTimeValue, @RequestParam("vitesse") String vitesseValue) {
        ...
        int waitTime = -1;
        if (waitTimeValue != null && !waitTimeValue.trim().isEmpty()) {
            try {
                waitTime = Integer.parseInt(waitTimeValue.trim());
            } catch (NumberFormatException ignored) {
            }
        }

        double vitesse = -1.0;
        if (vitesseValue != null && !vitesseValue.trim().isEmpty()) {
            try {
                vitesse = Double.parseDouble(vitesseValue.trim());
            } catch (NumberFormatException ignored) {
            }
        }

        try {
            AssignmentReport report = vehicleAssignmentService.buildDailyReport(selectedDate, waitTime, vitesse);
